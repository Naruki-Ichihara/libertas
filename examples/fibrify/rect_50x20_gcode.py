...
class F:
    ...
    threshold: float = 2          # path connection threshold (mm)
...
def build_layup(
    ...
    for spec in stacking:
        if isinstance(spec, F):
            # Generate fiber SVG (cached per angle)
            ...
            # Polymer needs any SVG with a contour — reuse the first fiber SVG,
            # or generate a 0° one as fallback for the contour outline.
            if not svg_cache:
